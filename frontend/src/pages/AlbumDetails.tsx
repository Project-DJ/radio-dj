...
const AlbumDetail = () => {
  ...
  const { data: album, isLoading: loadingAlbum, isError: errorAlbum } = useQuery({
  ...
  const { data: songs = [], isLoading: loadingSongs } = useQuery({
    ...
    queryFn: () => api.albums.songs(albumId),
    enabled: !!albumId,
  });
  ...
  const totalMs = songs.reduce((s, t) => s + t.duration_ms, 0);
  ...
  return (
    <div className="min-h-screen p-6 max-w-3xl mx-auto">
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-xs font-body text-muted-foreground">
              {songs.length} track{songs.length !== 1 && "s"}{totalMs > 0 ? ` · ${formatDuration(Math.floor(totalMs / 1000))}` : ""}
            </span>
          </div>
        </div>
      </motion.div>
      ...
      {songs.length === 0 ? (
        <div className="y2k-border y2k-shadow bg-card p-8 text-center">
          <p className="font-display text-sm uppercase text-muted-foreground">No songs in this album yet</p>
          <p className="text-xs font-body text-muted-foreground mt-1">Add songs via POST /songs/ with album: "{album.title}"</p>
        </div>
      ) : (
        <div className="y2k-border y2k-shadow bg-card overflow-hidden">
          {songs.map((song, i) => (
            ...
            >
              ...
              {song.bpm && (
                <span className="text-[10px] font-body text-muted-foreground mr-2">
                  {song.bpm} BPM
                </span>
              )}
              <span className="text-xs font-body text-muted-foreground">
          ...
          <div className="px-4 py-3 bg-y2k-blush border-t-[3px] border-foreground flex justify-between">
            ...
            <span className="font-display text-xs">{formatDuration(Math.floor(totalMs / 1000))}</span>
          </div>
        </div>
      )}
    </div>
